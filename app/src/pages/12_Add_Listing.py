...
# API endpoint
API_URL = "http://web-api:4000/seller-routes/listings"
...
with st.form(f"create_listing_{st.session_state.listing_counter}"):
    ...
    if submitted:
        ...
        if not all([product_name, category, description, price, condition, tags_raw]):
            ...
            product_data = {
                "SellerID": st.session_state.get("user_id", 1),  # Default to 1 if not set
                "Name": product_name,
            ...
            try:
                ...
                # Store product name for successful submission
